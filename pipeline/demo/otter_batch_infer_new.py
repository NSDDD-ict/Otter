...
def get_response(vision_x, prompt: str, model=None, image_processor=None, tensor_dtype=None, batch_size=2) -> str:
    ...
    generated_text = model.generate(
        ...
        pad_token_id=32003
        # pad_token_id=tokenizer.eos_token_id # 去掉会警告 The attention mask and the pad token id were not set. As a consequence, you may observe unexpected behavior. Please pass your input's `attention_mask` to obtain reliable results. Setting `pad_token_id` to `eos_token_id`:50277 for open-end generation.
    )
